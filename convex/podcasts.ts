...
export const getPodcastBySearch = query({
  ...
  handler: async (ctx, args) => {
    if (args.search === "") {
      return await ctx.db.query("podcasts").order("desc").collect(); // Return all podcasts if no search term is provided
    }
    ...
    if (authorSearch.length > 0) {
      return authorSearch; // Return results based on author's name match
    }
    ...
    if (titleSearch.length > 0) {
      return titleSearch; // Return results based on podcast title match
    }

    // Search by podcast description (or fallback to title)
    return await ctx.db
      .query("podcasts")
