...
const GenerateThumbnail = ({ setImage, setImageStorageId, image, imagePrompt, setImagePrompt }: GenerateThumbnailProps) => {
  ...
  const generateImage = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setIsImageLoading(true); // Set loading state to true while processing
    ...
    try {
      ...
      if (!response) {
        ...
        setIsImageLoading(false);
        return; // Stop execution without showing any error toast
      }

      const blob = new Blob([response], { type: 'image/png' });

      if (blob.size === 0) {
        console.log('Empty image blob received');
        setIsImageLoading(false);
        return; // Stop execution without showing any error toast
      }
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="w-full max-w-[200px]">
            <Button
              ...
              onClick={generateImage} // Call the image generation function
            >
              {isImageLoading ? (
                <>
                  Generating
